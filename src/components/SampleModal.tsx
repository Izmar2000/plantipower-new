...
const SampleModal: React.FC<SampleModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-[#011410] rounded-[30px] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] border border-[#0d2b24]">
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-6 right-6 z-50 p-2 rounded-full bg-[#0d2b24] text-white/50 hover:text-white transition-colors">
          <X className="w-5 h-5" />
        </button>
          ...
          <div className="overflow-y-auto custom-scrollbar">
            <div className="p-8 md:p-10">
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-8">
                {/* Product 1: All12 */}
                <div className="relative bg-[#0d2b24]/50 rounded-2xl p-6 border-2 border-lime-500/50 flex items-center justify-between group overflow-hidden">
                  <div className="relative z-10 flex items-center gap-4">
                    {/* Image Placeholder */}
                    <div className="w-12 h-16 bg-white/10 rounded-md flex items-center justify-center">
                      {/* Replace with actual image tag when paths are confirmed */}
                      <img src="/product-all12-small.png" alt="All12" className="h-full w-auto object-contain" onError={(e) => e.currentTarget.style.display = 'none'} />
                    </div>
                    <div>
                      <div className="text-[10px] text-lime-500 font-bold uppercase tracking-wider mb-1">{content.product1.sub}</div>
                      <div className="text-xl font-bold text-white leading-tight">{content.product1.name}</div>
                    </div>
                  </div>
                  <div className="absolute right-4 top-4 text-lime-500">
                    <div className="w-6 h-6 rounded-full bg-lime-500 text-[#011410] flex items-center justify-center">
                ...
                {/* Product 2: Shield */}
                <div className="relative bg-[#0d2b24]/50 rounded-2xl p-6 border-2 border-cyan-500/50 flex items-center justify-between group overflow-hidden">
                  <div className="relative z-10 flex items-center gap-4">
                    {/* Image Placeholder */}
                    <div className="w-12 h-16 bg-white/10 rounded-md flex items-center justify-center">
                      <img src="/product-shield-small.png" alt="Shield" className="h-full w-auto object-contain" onError={(e) => e.currentTarget.style.display = 'none'} />
                    </div>
                    <div>
                      <div className="text-[10px] text-cyan-400 font-bold uppercase tracking-wider mb-1">{content.product2.sub}</div>
                      <div className="text-xl font-bold text-white leading-tight">{content.product2.name}</div>
                    </div>
                  </div>
                  <div className="absolute right-4 top-4 text-cyan-500">
                    <div className="w-6 h-6 rounded-full bg-cyan-500 text-[#011410] flex items-center justify-center">
                      <Check className="w-4 h-4" />
                    </div>
                  </div>
                </div>
              ...
              {/* Total Price Block */}
              <div className="bg-[#021814] rounded-2xl p-6 border border-white/5 flex items-center justify-between mb-10">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-[#0d2b24] flex items-center justify-center text-lime-500">
                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                    </svg>
                  </div>
                  <div>
                    <div className="text-white font-bold uppercase tracking-wide">{content.bundleTitle}</div>
                    <div className="text-emerald-100/50 text-xs font-bold uppercase tracking-wider">{content.bundleSub}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-3xl font-bold text-white tracking-tight">{content.price}</div>
